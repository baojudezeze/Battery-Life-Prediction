...
X_cellsALL = np.zeros((1, 100))
Y_cellsALL = np.zeros((1, 100))
...
for f in files:
    ...
    for i in range(len(bat_dict)):
        ...
        if len(QC) >= 500:
            for j in range(100, 400, 1):  # len(QC) - 100
                X_slides.append(QC[j - 100:j])
                Y_slides.append(QC[j:j + 100])
        ...
        # for j in range(100, len(QC) - 50, 10):
        #     X_slides.append(QC[j-100:j])
        #     Y_slides.append(QC[j - 50:j + 50])
        # X_slides, Y_slides = np.array(X_slides), np.array(Y_slides)
        # X_cells = np.concatenate((X_cells, X_slides), axis=0)
        # Y_cells = np.concatenate((Y_cells, Y_slides), axis=0)
...
X_cellsALL = scaler.fit_transform(X_cellsALL)
Y_cellsALL = scaler.fit_transform(Y_cellsALL)
...
outputs = layers.Dense(units=100)(x)
# 构建模型
...
# model = Sequential()
# model.add(LSTM(100, input_shape=(inputs.shape[1], 1)))
# model.add(Dense(100))
...
# test = bat_dict_test['b1c0']['summary']['QC'][:100]
