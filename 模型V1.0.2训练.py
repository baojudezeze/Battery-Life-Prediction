...
if __name__ == "__main__":
    ...
    train_ds1 = tf.data.Dataset.from_tensor_slices(x1_train)
    train_ds1 = train_ds1.batch(batch_size).shuffle(1000)
    train_ds2 = tf.data.Dataset.from_tensor_slices(y_train)
    train_ds2 = train_ds2.batch(batch_size).shuffle(1000)
    # val_ds = tf.data.Dataset.from_tensor_slices((x_val, x1_val, x2_val, y_val))
    # val_ds = val_ds.batch(batch_size)
    val_ds = tf.data.Dataset.from_tensor_slices((x_val, x1_val, y_val))
    val_ds = val_ds.batch(batch_size)
    val_ds2 = tf.data.Dataset.from_tensor_slices(y_val)
    ...
    sample1 = next(iter(train_ds1))

    # 构造输入层，输入层要和x_train的shape一致,input_shape:(100,3,109,109)
    input_shape = sample.shape[1:]
    input_shape1 = sample1.shape[1:]
    # input_shape2 = sample[2].shape[1:]

    # 搭建模型
    model = models.build_model_108(input_shape, input_shape1, window)
